...
def transcribe_with_timestamps(audio_data, model, processor):
    ...
    device = "cuda"  # 或 model.device.type


    # 語音辨識模型
    whisperx_model = whisperx.load_model("medium", device=device, compute_type="float16")

    # 對齊模型
    alignment_model, metadata = whisperx.load_align_model("en", device)
    ...
    # 語音辨識
    transcribed_result = whisperx_model.transcribe(audio=audio_path, language="en")
    ...
    return {
        "text": " ".join([seg["text"] for seg in transcribed_result["segments"]]),
        "segments": aligned_result["word_segments"]
    }
